...
%keyboard;
[m,n]=size(d);
if 0, %n>m, Dont flip because there is difference in dimensions
    d = d';
    A = A';
    [m,n]=size(d);
    flip = 1;
...
x = inv(L')*(xt);
y = L*(yt+repmat(b,1,n));
%% Ett hack för att y ligger i 3D (men x ligger i 2D).
%keyboard;
x = [x;zeros(1,size(x,2))];
y = [y;zeros(1,size(y,2))];
dtmp=toa_calc_d_from_xy(x,y);
res00 = d-dtmp;
%hist(res00(:),100);
tmp = d.^2-dtmp.^2;
tmp = median(tmp);
