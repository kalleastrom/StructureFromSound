function [cmatches,cres] = estimate_sr_from_matcheso(bmatches,o,bres,settings);
...
%keyboard
badj = setdiff(find(min(cmatches.u)-o < 0.001),1);
cmatches.u(:,badj)=[];
cmatches.uok(:,badj)=[];
...
[sol,stats,bb,bb0] = toa_linear_fix(dtmp,cres.A);
xx = sol.x;
yy = sol.y;
oo = o;
if 1, % Checking for errors
...
[bb bb0 d2r(1,2:end)' d2rc(1,2:end)']
norm(bb+d2r(1,2:end)')
norm(bb0+d2rc(1,2:end)')
...
[xx2,yy2,oo2,res,jac]=bundletdoa(TT,I,J,xx,yy,oo,1,xinorm);
std(res)
[xx3,yy3,oo3,res,jac]=bundletdoa(TT,I,J,xx2,yy2,oo2,1,xinorm);
